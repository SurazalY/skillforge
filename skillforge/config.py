...
def compatibility_env_names(name):
    names = []
    if str(name).startswith("SKILLFORGE_"):
        names.append("PICO_" + str(name)[len("SKILLFORGE_"):])
    return tuple(names)


def provider_env(name, legacy_names=(), default=""):
    for env_name in (name, *compatibility_env_names(name), *legacy_names):
