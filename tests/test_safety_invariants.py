...
from skillforge import cli as mini_cli
...
def start_active_run(agent, *, user_request="exercise log tools"):
    state = TaskState.create(run_id="run_001", task_id="task_001", user_request=user_request)
...
def test_cli_build_agent_loads_project_env_secrets_before_redaction_setup(tmp_path):
    ...
    (tmp_path / "README.md").write_text("demo\n", encoding="utf-8")
    (tmp_path / ".env").write_text("SKILLFORGE_DEEPSEEK_API_KEY=sk-project-secret\n", encoding="utf-8")
    with patch.dict(os.environ, {}, clear=True), patch("skillforge.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = mini_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["SKILLFORGE_DEEPSEEK_API_KEY"]


def test_cli_build_agent_redacts_pico_compatibility_project_env_secrets(tmp_path):
    class DummyModelClient:
        def __init__(self, *args, **kwargs):
            self.args = args
            self.kwargs = kwargs
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=True,
    ), patch("skillforge.cli.OllamaModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = mini_cli.build_agent(args)
...
def test_log_run_is_denied_in_read_only_mode_and_cannot_mutate_workspace(tmp_path):
    agent = build_agent(tmp_path, [], approval_policy="never", read_only=True)
    start_active_run(agent, user_request="run pytest safely")
    test_file = tmp_path / "test_mutates_workspace.py"
    test_file.write_text(
        "from pathlib import Path\n"
        "\n"
        "def test_mutates_workspace():\n"
        "    Path('MUTATED.txt').write_text('created by pytest', encoding='utf-8')\n"
        "    assert True\n",
        encoding="utf-8",
    )
    ...
    result = agent.run_tool(
        "log_run",
        {"command": f"{shlex.quote(sys.executable)} -m pytest {shlex.quote(str(test_file))} -q", "timeout": 20},
    )

    assert result == "error: approval denied for log_run"
    assert not (tmp_path / "MUTATED.txt").exists()


def test_log_run_uses_allowlisted_environment_only_for_raw_logs(tmp_path):
    secret = "review-secret-should-not-leak"
    agent = build_agent(tmp_path, [], approval_policy="auto")
    start_active_run(agent, user_request="capture pytest log")
    test_file = tmp_path / "test_print_env.py"
    test_file.write_text(
        "import os\n"
        "\n"
        "def test_print_env():\n"
        "    print(os.getenv('SECRET_REVIEW_TOKEN', 'missing'))\n"
        "    assert True\n",
        encoding="utf-8",
    )

    with patch.dict(os.environ, {"SECRET_REVIEW_TOKEN": secret}, clear=False):
        result = agent.run_tool(
            "log_run",
            {"command": f"{shlex.quote(sys.executable)} -m pytest -s {shlex.quote(str(test_file))} -q", "timeout": 20},
        )

    evidence_id = re.search(r"evidence_id:\s*(\S+)", result).group(1)
    record = agent.current_evidence_store().get(evidence_id)
    raw_log = open(record.payload["raw_log_path"], encoding="utf-8").read()

    assert "status: passed" in result
    assert secret not in raw_log
    assert "missing" in raw_log


def test_log_run_still_executes_pytest_when_approval_allows_it(tmp_path):
    agent = build_agent(tmp_path, [], approval_policy="auto")
    start_active_run(agent, user_request="run pytest")
    ...
    assert "status: passed" in result
    assert "summary: 1 passed" in result


def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    agent = build_agent(tmp_path, [], approval_policy="auto")

    with patch("skillforge.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "skillforge.runtime"

    with patch("skillforge.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
