...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".skillforge").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".skillforge" not in result
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "skillforge/0.1"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "skillforge.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("skillforge.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_arg_parser_accepts_fake_provider(tmp_path):
    args = mini_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "fake"])

    assert args.provider == "fake"


def test_fake_provider_one_shot_writes_skillforge_artifacts(tmp_path, capsys):
    args = [
        "--cwd",
        str(tmp_path),
        "--provider",
    ...
    with patch.dict(os.environ, {"SKILLFORGE_FAKE_OUTPUTS": '["<final>Hello from fake.</final>"]'}, clear=True):
        exit_code = mini_pkg.main(args)

    captured = capsys.readouterr()
    assert exit_code == 0
    assert "Hello from fake." in captured.out
    assert len(list((tmp_path / ".skillforge" / "runs").glob("*/report.json"))) == 1
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "skillforge.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("skillforge.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("skillforge.cli.AnthropicCompatibleModelClient") as mock_anthropic:
    ...
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                ...
                "SKILLFORGE_DEEPSEEK_API_KEY=sk-project-deepseek",
...
def test_build_agent_keeps_pico_env_as_compatibility_fallback(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "PICO_DEEPSEEK_API_BASE=https://compat.deepseek.example/anthropic",
                "PICO_DEEPSEEK_API_KEY=sk-compat-deepseek",
                "PICO_DEEPSEEK_MODEL=compat-deepseek-model",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    assert mock_anthropic.call_args.kwargs["model"] == "compat-deepseek-model"
    assert mock_anthropic.call_args.kwargs["base_url"] == "https://compat.deepseek.example/anthropic"
    assert mock_anthropic.call_args.kwargs["api_key"] == "sk-compat-deepseek"


def test_skillforge_env_takes_precedence_over_pico_compatibility_env(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "PICO_DEEPSEEK_API_KEY=sk-compat-deepseek",
                "SKILLFORGE_DEEPSEEK_API_KEY=sk-skillforge-deepseek",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    assert mock_anthropic.call_args.kwargs["api_key"] == "sk-skillforge-deepseek"
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    runs_root = tmp_path / ".skillforge" / "runs"
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .skillforge/.\n"
    ...
    index_path = tmp_path / ".skillforge" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".skillforge" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".skillforge" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .skillforge/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".skillforge" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".skillforge" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(mini_pkg.__file__).as_posix().endswith("/skillforge/__init__.py")
